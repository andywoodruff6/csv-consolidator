...
def sum_cols_to_total(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add total columns for each non-date column showing cumulative sums.
    If a total column already exists, add new values to the existing totals.
    """
    df = df.copy()
    for col in df.columns:
        if col != 'Date' and 'total' not in col:
            total_col = f'{col} total'
            if total_col in df.columns:
                # Get the last total value (or 0 if empty)
                last_total = df[total_col].iloc[-1] if len(df[total_col]) > 0 else 0
                # Add new values to the existing total
                df[total_col] = last_total + df[col].cumsum()
            else:
                # Create new total column if it doesn't exist
                df[total_col] = df[col].cumsum()
    return df
...
def update_tracker(new_export_df: pd.DataFrame, tracker_path: Path) -> None:
    ...
    try:
        ...
        if tracker_df.empty:
            ...
            tracker_df = sum_cols_to_total(new_export_df)
        else:
            ...
            if len(new_rows) > 0:
                ...
                combined_df = pd.concat([tracker_df, new_rows])
                combined_df = sum_cols_to_total(combined_df)
                tracker_df = combined_df
            else:
